...
export function AddCameraWizard({ open, onClose, onCreated, defaultProtocol }: AddCameraWizardProps) {
  ...
  const [done, setDone]       = useState<{ id: string; rtmp_url?: string; stream_key?: string } | null>(null)
  ...
  const isLast = step === total - 1 && !done
  ...
  const handleCreate = async () => {
    ...
    try {
      const cam = await camerasSvc.create({
        ...
        rtsp_url: protocol === 'manual' ? form.manual_url : protocol === 'onvif' ? (connTest.rtsp_url ?? undefined) : undefined,
      ...
      if (protocol === 'rtmp_push') {
        try { const r = await camerasSvc.rtmpConfig(cam.id); rtmpInfo = r } catch { /* ok */ }
      }
      setDone({ id: cam.id, ...rtmpInfo })
    } catch (err: unknown) {
      type ApiErr = { response?: { data?: { detail?: string; message?: string } } }
      const data = (err as ApiErr)?.response?.data
      const msg = data?.detail || data?.message || 'Erro ao criar câmera'
      alert(msg)
    } finally {
      setLoading(false)
    }
  ...
  const handleClose = () => {
    ...
    setForm({ name: '', location: '', manufacturer: 'generic', retention_days: 5, stream_quality: 'high', onvif_ip: '', onvif_port: '80', onvif_username: 'admin', onvif_password: '', manual_url: '' })
    resetTest(); setHelpOpen(false); onClose()
  }
    ...
    <Modal
      ...
      footer={
          ...
          <div className="flex justify-between w-full">
            <button className="btn btn-ghost text-sm" onClick={step === 0 ? handleClose : () => { setStep(s => s - 1); resetTest() }} disabled={loading}>
            ...
            <div key={i} className="flex items-center flex-1 min-w-0">
              <div className="flex items-center gap-1.5 shrink-0">
                <div
                  ...
                  style={i < step ? { background: 'var(--accent)', color: '#fff' }
      ...
      {!done && step === 0 && (
        <div className="space-y-2">
          {([
            ...
            >
              <Icon size={16} className={protocol === id ? 'text-accent' : 'text-t3'} style={protocol === id ? { color: 'var(--accent)' } : {}} />
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <code className="flex-1 text-xs font-mono bg-elevated rounded-lg px-3 py-2 text-t1 break-all" style={{ background: 'var(--elevated)' }}>
                    {done.rtmp_url}
                  </code>
              ...
              {done.stream_key && (
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <code className="flex-1 text-xs font-mono bg-elevated rounded-lg px-3 py-2 text-t1" style={{ background: 'var(--elevated)' }}>
                      {done.stream_key}
                    </code>
